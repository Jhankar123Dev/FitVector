    ...
    <div className="mx-auto max-w-2xl space-y-5 py-4">
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">{report.jobTitle}</p>
          <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
            ...
            <span className="text-[11px] text-muted-foreground/70">~{report.durationPlanned} min</span>
          </div>
        </div>
      </div>
      ...
      {report.executiveSummary && (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-semibold text-foreground/80">Overall Summary</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            {report.skillRatings.map((sr, i) => (
              <div key={i} className="space-y-1">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-sm font-medium text-foreground/80">{sr.skill}</span>
                  <StarBar score={sr.score} />
                </div>
                {sr.note && (
                  <p className="text-[11px] text-muted-foreground leading-relaxed">{sr.note}</p>
                )}
              </div>
            ))}
          </CardContent>
          ...
          <CardContent className="space-y-3">
            {report.communicationScores.map((cs, i) => (
              <div key={i} className="space-y-0.5">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-sm text-foreground/80">{cs.label}</span>
                  <StarBar score={cs.score} />
                </div>
                {cs.note && (
                  <p className="text-[11px] text-muted-foreground">{cs.note}</p>
                )}
              </div>
            ))}
          </CardContent>
